...
constexpr auto is_char(char c) noexcept -> bool { return c >= 'A' && c <= 'Z'; }
} // namespace

auto termhack::detail::interaction_logic::look_at(
    std::size_t index, const terminal_buffer &terminal,
    game_state &state) noexcept -> highlight {
  ...
  if (is_open_bracket(terminal.get(index)) &&
      !state.is_bracket_used(index)) // case of brackets
  {
...
auto termhack::detail::interaction_logic::_internal::click_at(
    std::size_t index, terminal_buffer &terminal, word_repository &words,
    game_state &state, rng_fref gen_next) noexcept -> click_status {
  assert(index < terminal.size());
    ...
    if (words.is_word_offset(b)) // word
    {
      ...
      } else // it's not an answer
      {
        ...
        const std::size_t match = std::inner_product(
            ...
            term.begin() + words.offsets()[words.answer_index()], 0,
        ...
        if (state.attempts_left() == 0) {
          state.set_game_over();
          return {click_result::lockout_in_progress,
                  word_match{match, words.word_length()}};
        }

        return {click_result::entry_denied,
                word_match{match, words.word_length()}};
      }
    } else // bracket
    {
      const std::size_t dist = std::distance<const char *>(
          term.data(),
        ...
        } else // wordsleft != 1
        {
          if (state.attempts_left() == 4) {
            ...
            for (auto offset : words.offsets()) {
              ...
              if (!words.is_removed(offset) &&
                  !words.is_answer_offset(offset)) {
                --dud_number;
              }
            }

            terminal.replace_with_char(dud_number,
                                       dud_number + words.word_length());
