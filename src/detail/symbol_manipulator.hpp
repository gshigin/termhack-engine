...
template <class URBG>
  requires std::uniform_random_bit_generator<std::remove_reference_t<URBG>>
auto generate_words(std::size_t length, std::size_t count,
                    URBG &&g) noexcept -> std::array<std::string, 20> {
  return _internal::generate_words(length, count,
                                   _internal::wrap_rng(std::forward<URBG>(g)));
}

template <class URBG>
  requires std::uniform_random_bit_generator<std::remove_reference_t<URBG>>
void generate_term_chars(terminal_buffer &terminal, URBG &&g) noexcept {
  return _internal::generate_term_chars(
      terminal, _internal::wrap_rng(std::forward<URBG>(g)));
}
...
auto place_words(terminal_buffer &terminal,
  ...
  return _internal::place_words(terminal, words, length, count,
                                _internal::wrap_rng(std::forward<URBG>(g)));
}
