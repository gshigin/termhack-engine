...
auto compute_word_count(termhack::lock_level lock_level_setting,
  ...
  const std::size_t lock_level =
      static_cast<std::size_t>(lock_level_setting) * 25;
  ...
  const double sol_coef =
      lock_offset == 0
          ...
          : (1 - (static_cast<double>(science_offset) /
                  static_cast<double>(
                      lock_offset))); // science_offset over lock_offset coef

  const std::size_t word_count_ =
      static_cast<std::size_t>(sol_coef *
                               (hacking_max_words - hacking_min_words)) +
      hacking_min_words;
...
inline auto compute_word_length(termhack::lock_level lock_level_setting,
                                URBG &&g) noexcept -> std::size_t {
  return std::min<std::size_t>(
      12, 4 + 2 * static_cast<std::size_t>(lock_level_setting) +
              (std::forward<URBG>(g)() & 1));
}
} // namespace
...
struct engine::impl {
  impl(lock_level lock_level_setting, uint32_t player_science_skill,
       uint64_t seed) noexcept
      : rng{static_cast<uint32_t>(util::seed(seed))} {
    detail::symbol_manipulator::generate_term_chars(terminal, rng);

    const std::size_t word_length =
        compute_word_length(lock_level_setting, rng);
    const std::size_t word_count =
        compute_word_count(lock_level_setting, player_science_skill);

    const std::array<std::string, 20> word_list =
        detail::symbol_manipulator::generate_words(word_length, word_count,
                                                   rng);
    const std::size_t answer_index = rng.next() % word_count;
    const std::array<uint16_t, 20> offsets =
        detail::symbol_manipulator::place_words(terminal, word_list,
                                                word_length, word_count, rng);

    words.init(word_length, word_count, answer_index, offsets);
  }

  auto check_coords(screen_coords coords) const noexcept
    ...
    if ((8 <= m_x && m_x <= 19) && (6 <= m_y && m_y <= 22)) // first window
    {
      m_x -= 8;
      m_y -= 6;
    } else if ((28 <= m_x && m_x <= 39) &&
               (6 <= m_y && m_y <= 22)) // second window
    {
      m_x -= 28;
      m_y -= 6;
      m_y += 17;
  ...
  auto process_input(input current_input) noexcept -> state {
    ...
    case input_type::look:
      if (internal_coord.has_value()) {
        auto hl = detail::interaction_logic::look_at(*internal_coord, terminal,
                                                     state);
    ...
    case input_type::click:
      if (internal_coord.has_value()) {
        auto click_res = detail::interaction_logic::click_at(
            *internal_coord, terminal, words, state, rng);
        return {terminal.view(), state.attempts_left(), std::nullopt,
                click_res};
      }
      return {terminal.view(), state.attempts_left(), std::nullopt,
...
engine::engine(const lock_level lock_level_setting,
               const uint32_t player_science_skill,
               const uint64_t seed = 0) noexcept
    : pimpl_(std::make_unique<impl>(lock_level_setting, player_science_skill,
                                    seed)) {}
